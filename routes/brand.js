...
router.post('/add',upload.single('brandImg'),function(req,res){
    ...
    fs.readFile(req.file.path,function(err,filedata){
        if(err){
            console.log('读取文件失败');
            res.send({colde: -1,msg:'新增品牌失败'});
        } else{
            var fileName = new Date().getTime()+'_'+req.file.originalname;
            var dest_path = path.resolve(__dirname,'../public/brand/',fileName);
                                ...
                                function(callback){
                                 ...
                                 function(callback){
                                    if(brandNum <= 0){
                                        ...
                                        db.collection('brand').find().skip(brandNum-1).toArray(function(err,data){
                                            if (err) {
...
router.get('/delete',function(req,res){
    MongoClient.connect(url,function(err,client) {
        if (err) {
            ...
            var db = client.db('bao');
            db.collection('brand').deleteOne({'_id':parseInt(req.query._id)},function(err){
                if(err){
                    console.log('删除数据失败');
...
router.post('/updata',upload.single('brandImg'),function(req,res){
    ...
    if(saveData.rewriteImg){
        ...
        MongoClient.connect(url,function(err,client){
            if(err){
                ...
                db.collection('brand').update( {'_id' : parseInt(saveData.brandId)} ,{$set:{
